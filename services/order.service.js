...
const placeOrder = async (userId) => {
    ...
    try {
        ...
        for (const item of cart.items) {
            ...
            if (!game) {
                throw new Error(`Game with ID ${item.game._id} not found. Cannot place order.`);
            }

            if (game.stock < item.quantity) { //
                throw new Error(`Not enough stock for "${game.title}". Available: ${game.stock}, Requested: ${item.quantity}`); //
            }
            ...
            await Game.findByIdAndUpdate(game._id, { $inc: { stock: -item.quantity } }, { session }); //
        }
